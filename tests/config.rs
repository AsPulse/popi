...
fn loading_no_paths_config_file() {
  ...
  assert_eq!(config.err().unwrap(), LoadConfigError::NoPathsConfigFileFound);
}
...
fn loading_correct_paths() {
  ...
  assert_eq!(config.repo_paths[0].to_str().unwrap(), "/Users/aspulse/repositories");
  assert_eq!(config.repo_paths[1].to_str().unwrap(), "/Users/aspulse/github");
}

#[test]
fn loading_broken_config() {
  let config = Config::new_from_config_path("tests/fixtures/config_2".into());
  assert_eq!(config.err().unwrap(), LoadConfigError::PathConfigInvalidYamlFormat);
}
